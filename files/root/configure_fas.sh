...
# Set up WAN DNS first
uci set network.wan.dns='8.8.8.8 8.8.4.4'  # Google DNS servers
uci set network.wan.peerdns='0'            # Don't accept DNS from DHCP

# Configure DNSMasq to use Google DNS as upstream
uci del_list dhcp.@dnsmasq[0].server='8.8.8.8' # Remove if exists
uci add_list dhcp.@dnsmasq[0].server='8.8.8.8' # Add as main upstream DNS

# Special DNS entries for FAS and status client
...
uci set network.@route[-1].metric='1'
